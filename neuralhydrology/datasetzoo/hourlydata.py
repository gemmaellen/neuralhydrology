...
class HourlyData(dailydata.DailyData):
    """Data set class providing hourly data. Adapted from hourlycamelsus.py
    
    This class extends the `DailyData` dataset class by hourly in- and output data. Currently, only NLDAS forcings are
    ...
    def __init__(self,
        ...
        super(HourlyData, self).__init__(cfg=cfg,
                                             is_train=is_train,
                                             period=period,
                                             basin=basin,
    ...
    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        ...
        for forcing in self.cfg.forcings:
            if forcing[-7:] == '_hourly':
                ...
                df, _ = dailydata.load_daily_forcings(self.cfg.data_dir, basin, forcing)
        ...
        #     attributes = dailydata.load_camels_us_attributes(data_dir=self.cfg.data_dir, basins=[basin])
